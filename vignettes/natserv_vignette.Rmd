---
title: "Introduction to `natserve` - An R package to wrap NatureServe's database API from rOpenSci"
author: Mairin Deith
output: 
   html_document:
     toc: true
...
#### 2. Define the API key for all scripts in your RProject
...
But let's say you're interested in not only all members of the same genus, but a variety of species that can't be searched using a simple wild card.
To search for multiple taxa, you can make a vector of search terms containing the scientific names of the species you're interested in, then search each one in a loop or use the `lapply` function:
```{r ns.search.multisp}
...
Sticking to the gray wolf example, let's pull up what data are available for *Canis lupus* using the UID from `ns_search`. Note that you can use a call
...
A lot of information is returned by `ns_data` - it might take some time to pull it out of the nested list structure. 
You can view all of the data available to you by using the `str()` command:
...
# This returns a list of lists, with each entry the species' conservation status in a given state (listed alphabetically)
